...
scoreFile = './results20150615_TwoClass.csv'
resultFolder = './'
...
if __name__ == '__main__':
    ...
    for framework in ["SelfTrainTest"]:
        for model in ["WM", "OLDM", "OM"]:
            configList = genConfig(defaultConfig[model], iterConfig[model], nameList[model], prefix = model + "_TwoClass")
    ...
    for framework in ["SelfTrainTest"]:
        for model in ["OLDM", "OM"]:
            configList = genConfig(defaultConfig[model], iterConfig[model], nameList[model], prefix = "WM_" + model + "_TwoClass")
            print(len(configList))
            for name, config in configList:
    ...
    for framework in ["SelfTrainTest"]:
        for model in ["OM"]:
            configList = genConfig(defaultConfig[model], iterConfig[model], nameList[model], prefix = "WM_OLDM_" + model + "_TwoClass")
            print(len(configList))
            for name, config in configList:
                #cmd = 'python3 ./clean.py %s_results.csv %s_results2.csv' % (name, name)
                cmd = 'python3 ./CollectResult.py testScore %s %s %s/%s_results.csv 0 6 >> %s' %(framework, name, resultFolder, name, scoreFile)
