...
//get feature vector given seqLabel, nodeIndex and preNodeIndex
void RawFeatures::getFeatures(featureVector& listFeatures, DataSequence* X, Model* m, int nodeIndex, int prevNodeIndex, int seqLabel)
{
	if(X->getPrecomputedFeatures() != NULL && prevNodeIndex == -1)
	{
		...
		for(int s = 0; s < nbStateLabels; s++)
		{
			...
			{
				pFeature = listFeatures.addElement(); //feature vector +1 dimension
				...
				pFeature->prevNodeIndex = -1;  //don't need previous node ?
...
void RawFeatures::init(DataSet& dataset, const Model& m)
{
