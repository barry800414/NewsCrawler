...
//for including additional resource
#include "data.h"
...
public:
    ...
    //for including additional resource
	void setSents(const std::vector<Sentence>& x);
	Sentence getSent(int nodeIndex);

	//void setSent(const Sentence& value, unsigned int nodeIndex);
